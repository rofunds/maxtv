...
#MOVIE VAULT Add-on Created By Mucky Duck (3/2016)

User_Agent = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2272.89 Safari/537.36'
addon_id='plugin.video.mdmovievault'
...
def CAT():
        addDir('[B][COLOR ivory]Popular Movies[/COLOR][/B]',baseurl,1,icon,fanart,'')
        addDir('[B][COLOR ivory]Recent Movies[/COLOR][/B]',baseurl+'/recent_movies',1,icon,fanart,'')
        addDir('[B][COLOR ivory]Most Viewed[/COLOR][/B]',baseurl+'/most_viewed',1,icon,fanart,'')
        addDir('[B][COLOR ivory]Search[/COLOR][/B]','url',4,icon,fanart,'')
        addDir('[B][COLOR ivory]Genre[/COLOR][/B]',baseurl,2,icon,fanart,'')
        addDir('[B][COLOR ivory]Year[/COLOR][/B]',baseurl,5,icon,fanart,'')
...
def INDEX(url):
        ...
        for a in all_videos:
                ...
                if metaset=='true':
                        addDir2('[B][COLOR ivory]%s[/COLOR][/B]' %name,baseurl+url,3,'http://'+thumb,items)
                else:
                        addDir('[B][COLOR ivory]%s[/COLOR][/B]' %name,baseurl+url,3,'http://'+thumb,fanart,'')
        try:
                match = re.compile('<a href="(.*?)\?page\=(.*?)">').findall(link)
                for url, pn in match:
                        url = baseurl+url+'?page='+pn
                        addDir('[I][B][COLOR lightskyblue]Page %s >>>[/COLOR][/B][/I]' %pn,url,1,icon,fanart,'')
        except: pass
...
def LINK(url):
        ...
        headers = {'origin':'http://fmovie.co', 'referer': url,
...
def GENRE(url):
        ...
        for a in all_videos:
                ...
                url = regex_from_to(a, 'href="', '"').replace("&amp;","&")
                addDir('[B][COLOR ivory]%s[/COLOR][/B]' %name,baseurl+url,1,icon,fanart,'')
...
def addDir2(name,url,mode,iconimage,itemcount):
        name = name.replace('[B][COLOR ivory]','').replace('[/COLOR][/B]','')
        ...
        if meta['cover_url']=='':
            try:
                ...
                meta['cover_url']=icon
        name = '[B][COLOR ivory]' + name + '[/COLOR][/B]'
        meta['title'] = name
        contextMenuItems = []
        if meta['trailer']>'':
