...
class cartoonhd(MovieSource,TVShowSource):
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):                 
       ...
       if type == 'tv_episodes':
           ...
           RESULT_TERM = '%s' %(name.lower())
           try:GOOGLED = self.GoogleSearch('cartoonhd.mobi', search_term)
           except:GOOGLED = bing.Search('cartoonhd.mobi', search_term)
       ...
       for result in GOOGLED:          
           ...
           if RETURN.lower() in TITLE.lower():
               if movie_url not in uniques:
                   ...
                   if type == 'tv_episodes':
                       if 'season/%s/episode/%s' % (season,episode) in movie_url:

                           self.GetFileHosts( movie_url, list, lock, message_queue,type)
