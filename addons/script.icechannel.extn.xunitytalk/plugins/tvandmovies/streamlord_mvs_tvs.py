...
class streamlord(MovieSource,TVShowSource):
    ...
    def Resolve(self, url):
            ...
            stream=re.compile('true, "file": "(.+?)"').findall(result)[0]
            final=stream.split('://')[1]
            host=final.split('/')[0]
            cookie=re.compile('cf_clearance="(.+?)"').findall(open(self.cookie_file).read())[0]
            cfduid=re.compile('cfduid=(.+?);').findall(open(self.cookie_file).read())[0]
            
            AND='|Host=%s&User-Agent=%s&Referrer=%s&Connection=keep-alive&Cookie=cf_clearance=%s; cfduid=%s' %(host,UA,url,cookie,cfduid)           
