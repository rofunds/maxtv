...
class playbox(MovieSource,TVShowSource):
    ...
    def GetFileHosts(self, url, list, lock, message_queue,season,episode,title):
        ...
        new_url = 'http://playboxhd.com/api/box?type=detail&id=%s&os=Android&v=4.10&k=0&al=key' % (url)
        ...
        new_url = 'http://playboxhd.com/api/box?type=stream&id=%s&os=Android&v=4.10' % ID
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):                 
        ...
        main_url= self.API+'type=search&os=Android&v=4.10&k=0&keyword=%s' %(name.replace(' ','+'))
        ...
        for field in data['films']:
            ...
            if name.lower() in NAME.lower():
                self.GetFileHosts(item_url, list, lock, message_queue,season,episode,title)
