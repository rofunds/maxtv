...
class CPlayList:
    ...
    def load_plx(self, filename='', mediaitem=CMediaItem(), proxy="CACHING"):
            ...
            if filename != '':
                    ...
                    if self.URL == 'My Playlists': self.URL = xbmc.translatePath(os.path.join(myPlaylistsDir,"My Playlists.plx"))
                    if self.URL == 'My Playlists.plx': self.URL = xbmc.translatePath(os.path.join(myPlaylistsDir,"My Playlists.plx"))
                    if self.URL == myPlaylistsDirB: self.URL = xbmc.translatePath(myPlaylistsDir)
                    #if self.URL == xbmc.translatePath(os.path.join(myPlaylistsDirB,'My Playlists')): self.URL = xbmc.translatePath(os.path.join(myPlaylistsDir,"My Playlists.plx"))
                    print self.URL
            else:
                    self.URL = mediaitem.URL
                    try:
                      if ('/Playlists/scripts.plx' in self.URL): 
                        if ('offshoregit.com/navixtreme/navi-x-support-files/' in self.URL) or ('github.com/HIGHWAY99/navi-x-storage-unit/' in self.URL): 
                          if tfalse(SettingG("scripts-testers"))==True:
                            if SettingG("scripts-testers2").lower()==("In love with XBMCHUB").lower():
                              self.URL=self.URL.replace('/Playlists/scripts.plx','/Playlists/scripts-testers.plx')
    ...
    def load_dir(self, filename='', mediaitem=CMediaItem(), proxy="CACHING"):
            ...
            if self.URL[:3] != 'ftp':
                    ...
                    if self.URL[1] != ':':
                            ...
                            if self.URL=='myxbmc.plx': self.URL = RootDir + self.URL
                            if self.URL=='history.plx': self.URL = RootDir + self.URL
                            if self.URL=='blacklist.plx': self.URL = RootDir + self.URL
                            if self.URL=='downlmenu.plx': self.URL = RootDir + self.URL
                            if self.URL=='downloads.plx': self.URL = RootDir + self.URL
                            if self.URL=='downlqueue.plx': self.URL = RootDir + self.URL
                            #if self.URL=='favorites.plx': self.URL = xbmc.translatePath(os.path.join(datapaths,self.URL))
                            #if self.URL=='favorites': self.URL = xbmc.translatePath(os.path.join(datapaths,self.URL))
                            #if self.URL=='My Playlist': self.URL = xbmc.translatePath(os.path.join(datapaths,self.URL))
                            #if self.URL=='My Playlist.plx': self.URL = xbmc.translatePath(os.path.join(datapaths,self.URL))
